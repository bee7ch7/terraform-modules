...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_s3"></a> [s3](#module\_s3) | ./s3_bucket | n/a |
...
| <a name="input_create_bucket"></a> [create\_bucket](#input\_create\_bucket) | Whether to create S3 bucket for Cloudfront distribution | `bool` | `true` | no |
...
| <a name="input_front_bucket_name"></a> [front\_bucket\_name](#input\_front\_bucket\_name) | Whether to create S3 bucket for Cloudfront distribution | `string` | n/a | yes |
...
locals {
  ...
  bucket_name        = "${local.environment}.${local.domain_name}"

  name = "syncsoul"
}

terraform {
  source = "git::git@github.com:syncsoul/terraform-modules.git//aws/s3_cloudfront?ref=main"
}
...
inputs = {
  front_bucket_name = local.bucket_name
  name              = local.name
  environment       = local.environment

  comment             = "SyncSoul Cloud Front"
  ...
  origin = {
    "s3_oac_${local.environment}" = {
      ...
      origin_access_control = "s3_oac_${local.environment}"
    }
  }

  default_cache_behavior = {
    target_origin_id       = "s3_oac_${local.environment}" # key in `origin` above
  ...
  viewer_certificate = {
    acm_certificate_arn = dependency.acm.outputs.acm_prime_arn
  ...
  tags = {
    Name        = "${local.name}-distribution-${local.environment}",
    Environment = local.environment
  }
}
```
